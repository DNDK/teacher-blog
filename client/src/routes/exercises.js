...
class Exercises extends Component {
    ...
    componentDidMount() {
        axios.get(`/api/exercises?page=${this.state.page - 1}`).then(res => {
    ...
    renderLink(article) {
        ...
        return (
            <div className="article-link">
                <div><h3><Link to={`/exercises/${article._id}`}>{article.title}</Link></h3></div>
                <div className="preview">
                    {preview}<br />
                    <Link to={`/exercises/${article._id}`} className="read-more">Читать дальше</Link>
                </div>
