...
class Consultations extends Component {
    ...
    componentDidMount() {
        axios.get(`/api/consultations?page=${this.state.page - 1}`).then(res => {
    ...
    renderLink(article) {
        ...
        return (
            <div className="article-link">
                <div><h3><Link to={`/consultations/${article._id}`}>{article.title}</Link></h3></div>
                <div className="preview">
                    {preview}<br />
                    <Link to={`/consultations/${article._id}`} className="read-more">Читать дальше</Link>
                </div>
